...
(function(global) {
  ...
  function toUrl(moduleId) {
    if(_reAbsPath.test(moduleId))
      return moduleId;

    return _rootModule.get(moduleId).packageMainOrSelf().pathLoad(/* omitExtension: */true);
  }
  ...
  // Can be configured through "loadSync" config option as well.
  ...
  Module.prototype.getRequire = function() {
    ...
    contextualRequire.toUrl = function(moduleId) {
      if(_reAbsPath.test(moduleId))
        return moduleId;

      return ctxModule.get(moduleId).packageMainOrSelf().pathLoad(/* omitExtension: */true);
    };

    return contextualRequire;

    function contextualRequire() {
  ...
  Module.prototype.packageMainOrSelf = function() {
    return this.packageMain || this;
  };

  // The path for loading purposes (and not for parent child path concatenation, etc.)
  Module.prototype.pathLoad = function(omitExtension) {
    var path = this.path();
    ...
    return omitExtension ? path : (path.replace(_reJsExt, '') + '.js');
  };
  ...
  Module.prototype.configPackage = function(pkg) {
    ...
    this.packageMain = this.get('./' + this.name + '/' + main);

    if(pkg.location) this.configPath(pkg.location);
  };
  ...
  Module.prototype.get = function(mid, expectsAbsolute) {
    ...
    var module = !isRelative ? this.shared.root : (this.parent || this); // root can't go further up
  ...
  Module.prototype.requireOne = function(mid, inDefine) {
    ...
    // Load urls directly
    if(_reAbsPath.test(mid)) {
      return void this._loadPath(mid);
    }
    ...
    var module = this.get(mid).packageMainOrSelf();
    return isPlugin ? module.pluginLoad(data, /* fromModule: */this) : module.resolve();
  };
  ...
  Module.prototype.pluginLoad = function(id, fromModule) {
    ...
    if(plugin) {
      ...
      if(typeof load === 'function') {
        load.call(plugin, id, fromModule.getRequire(), function(value) {
          exports = value;
        }, {isBuild: false});
